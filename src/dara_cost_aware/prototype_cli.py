...
from .prototype_logic import PILOT_BUDGETS, PrototypeScenario, run_four_arm_prototype
from .scheduling import PreparedExpansion, PreparedSibling


def _missing_siblings(action_id: str, count: int) -> tuple[PreparedSibling, ...]:
    return tuple(
        PreparedSibling(
    ...
    return PrototypeScenario(
        ...
        actions=(
            PreparedExpansion(
                ...
                dara_benefit=8,
                v3_benefit=4,
                siblings=_missing_siblings("fifo-high-dara", 4),
            ),
            PreparedExpansion(
                ...
                dara_benefit=3,
                v3_benefit=9,
                siblings=_missing_siblings("cheap-high-v3", 1),
            ),
            PreparedExpansion(
                action_id="v3-top",
                parent_id="node-002",
                ...
                dara_benefit=2,
                v3_benefit=12,
                siblings=_missing_siblings("v3-top", 3),
            ),
            PreparedExpansion(
...
def main() -> None:
    ...
    for run in runs:
        print(
            f"\nRUN arm={run.arm.value} budget={run.budget} "
            f"frozen_state={run.frozen_state_id}"
        )
                ...
                _json_line(
                    {
                        ...
                        "validated_cache_keys_after": sorted(
                            transition.validated_cache_keys_after
                        ),
                    }
                )
        _json_line(
